...
export function EmailCapture({ placement = 'inline', headline, subline }: EmailCaptureProps) {
  ...
  const successBox = (
    ...
    >
      ...
      <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>Your email was saved successfully.</p>
    </div>
  )
  ...
  const formBox = (dark = false) => (
    ...
    >
      ...
      <p
        style={{
          opacity: dark ? 0.9 : 1,
          color: dark ? 'inherit' : 'var(--text-secondary)',
      ...
      <form
        ...
        style={{ display: 'flex', gap: '0.5rem', maxWidth: '420px', margin: '0 auto', flexWrap: 'wrap', justifyContent: 'center' }}
      >
        <input
          type="email"
          required
        ...
        <button
          ...
          style={{
            ...
            background: dark ? '#1a1a2e' : 'var(--accent)',
            color: '#fff',
            border: 'none',
            borderRadius: '0.5rem',
            fontWeight: 700,
      ...
      {status === 'error' && <p style={{ marginTop: '0.75rem', fontSize: '0.85rem', opacity: 0.9 }}>{errorMessage}</p>}
    </div>
  )
    ...
    return (
      ...
      >
        <div
          ...
          style={{ background: 'var(--bg)', borderRadius: '1rem', padding: '2.5rem', maxWidth: '480px', width: '100%', position: 'relative', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}
        >
          <button
            onClick={() => setPopupOpen(false)}
            style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'none', border: 'none', fontSize: '1.25rem', cursor: 'pointer', color: 'var(--text-muted)' }}
          >
            x
          </button>
          <div style={{ textAlign: 'center', marginBottom: '0.5rem', fontSize: '2.5rem' }}>Mail</div>
