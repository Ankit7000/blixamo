...
export function AuthorBio({ name, compact = false }: AuthorBioProps) {
  ...
  const authorHref = `/author/${encodeURIComponent(name.toLowerCase())}`

  if (compact) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
        <Link href={authorHref} aria-label={`View ${name} author page`} style={{ display: 'block' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '50%', overflow: 'hidden', flexShrink: 0, border: '2px solid var(--accent)' }}>
            <Image src="/images/author-avatar.svg" alt={name} width={32} height={32} style={{ objectFit: 'cover', width: '100%', height: '100%' }} />
          </div>
        </Link>
        <div>
          <div style={{ fontSize: '0.875rem', fontWeight: 600 }}>
            <Link href={authorHref} style={{ color: 'var(--text-primary)' }}>{name}</Link>
          </div>
    ...
    <div style={{
      ...
      <Link href={authorHref} aria-label={`View ${name} author page`} style={{ display: 'block' }}>
        <div style={{ width: '56px', height: '56px', borderRadius: '50%', overflow: 'hidden', flexShrink: 0, border: '2px solid var(--accent)' }}>
          <Image src="/images/author-avatar.svg" alt={name} width={56} height={56} style={{ objectFit: 'cover', width: '100%', height: '100%' }} />
        </div>
      </Link>
      <div>
